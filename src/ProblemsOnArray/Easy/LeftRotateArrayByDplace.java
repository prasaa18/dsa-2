...
public class LeftRotateArrayByDplace {
  //r bruteforce o(d)+o(n-d)+o(d) --o(n+d)
     //sc --o(d) extra space temp array
    public static void Rotatetoleft(int[] arr, int n, int k) {
        ...
        if (k > n)
            return;
        int[] temp = new int[k];
        for (int i = 0; i < k; i++) {  // add to temp arraty
            temp[i] = arr[i];
        }
        for (int i = 0; i < n - k; i++) {  /// shjift the arraty
            arr[i] = arr[i + k];
        }
        for (int i = n - k; i < n; i++) { //add temp back last array
            arr[i] = temp[i -( n - k)];
        }
    }
